...
class IR:
    ...
    def __call__(self, num_players):
        ...
        #Splits
        # if kmean == 3:
        #     data_y = KMeans(n_clusters=num_players+3).fit_predict(data)
        #     players = [[] for _ in range(num_players+3)]
        # elif kmean == 2:
        #     data_y = KMeans(n_clusters=num_players+2).fit_predict(data)
        #     players = [[] for _ in range(num_players+2)]
        # elif kmean == 1:
        #     data_y = KMeans(n_clusters=num_players+1).fit_predict(data)
        #     players = [[] for _ in range(num_players+1)]
        # else:
