...
#Chip Measurments in mm
chipW=40
chipH=40
chip_cornerXmin=2
chip_cornerXmax=10.5
chip_cornerYmin=2.5
chip_cornerYmax=23
...
chipW*=zoom
chipH*=zoom
chip_cornerXmin=int(chip_cornerXmin*zoom)
chip_cornerXmax=int(chip_cornerXmax*zoom)
chip_cornerYmin=int(chip_cornerYmin*zoom)
chip_cornerYmax=int(chip_cornerYmax*zoom)
...
def display_img(img,polygons=[],channels="bgr",size=9):
    ...
    if channels != 1: ax.imshow(img)
    else: ax.imshow(img, cmap='gray')
    for polygon in polygons:
        #print("polygons = ", type(polygon))
...
#Chip Width and Height idk what values
chip_imgW=720
chip_intH=720
...
#chip reference
refChip=np.array([[0,0], [chipW,0], [chipW, chipH], [0, chipH]], dtype=np.float32)


#card references with bounding boxes
...
# #give a random background
# _=backgrounds.get_random(display=True)


#gives the total card size
...
#total chip size
bord_size=2 # bord_size alpha=0
chip_alphamask=np.ones((chipH,chipW),dtype=np.uint8)*255
#cv2.rectangle(chip_alphamask,(0,0),(chipW-1,chipH-1),0,bord_size)
cv2.circle(chip_alphamask, (80,80), 100, (0,0,0), thickness=40)
# cv2.line(chip_alphamask,(bord_size*3,0),(0,bord_size*3),0,bord_size)
# cv2.line(chip_alphamask,(chipW-bord_size*3,0),(chipW,bord_size*3),0,bord_size)
# cv2.line(chip_alphamask,(0,chipH-bord_size*3),(bord_size*3,chipH),0,bord_size)
# cv2.line(chip_alphamask,(chipW-bord_size*3,chipH),(chipW,chipH-bord_size*3),0,bord_size)
plt.figure(figsize=(10,10))
plt.imshow(chip_alphamask)
plt.show()
...
def extract_card (img, output_fn=None, min_focus=120, debug=False):
    ...
    _, edge = cv2.threshold(gray, 100, 255, cv2.THRESH_BINARY)
    # thres = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 21, 4)
    cnts, _ = cv2.findContours(edge.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # We suppose that the contour with largest area corresponds to the contour delimiting the card
    cnt_sorted = sorted(cnts, key = cv2.contourArea, reverse = True)
    cnt = cnt_sorted[0]
    ...
    print("AREA COUNT = ", areaCnt)
    print("AREA BOX = ", areaBox)
    ...
    if debug:
        ...
        display_img(gray, channels=1)
        display_img(edge, channels=1)
...
def extract_chip (img, output_fn=None, min_focus=120, debug=False):
    ...
    # Find the contours in the edged image
    _, edge = cv2.threshold(gray, 80, 255, cv2.THRESH_BINARY_INV)
    # thres = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 21, 4)
    cnts, _ = cv2.findContours(edge.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    if valid:
        ...
        # Determine 'Mp' the transformation that transforms 'box' into the reference rectangle
        if wr>hr:
            Mp=cv2.getPerspectiveTransform(np.float32(box),refChip)
        else:
            Mp=cv2.getPerspectiveTransform(np.float32(box),refChip)
        # Determine the warped image by applying the transformation to the image
        imgwarp=cv2.warpPerspective(img,Mp,(chipW,chipH))
        # Add alpha layer
        imgwarp=cv2.cvtColor(imgwarp,cv2.COLOR_BGR2BGRA)
        ...
        # First, initialize alpha channel fully transparent
        alphachannel=np.zeros(imgwarp.shape[:2],dtype=np.uint8)
        # Then fill in the contour to make opaque this zone of the chip 
        cv2.drawContours(alphachannel,cntwarp,0,255,-1)
        
        # Apply the alphamask onto the alpha channel to clean it
        alphachannel=cv2.bitwise_and(alphachannel,chip_alphamask)
        
        # Add the alphachannel to the warped image
        imgwarp[:,:,3]=chip_alphamask
...
#Pickle File
imgs_dir="pics"
cards={value:[] for value in card_values}
for suit in card_suits:
    for value in card_values:
        card_name=value+suit+".JPG"        
        card_name=os.path.join(imgs_dir,card_name)
        ...
        img=cv2.imread(card_name,cv2.IMREAD_UNCHANGED)
        _, img = extract_card(img, None, 120, False)
        print(img.shape)
        ...
        img=cv2.cvtColor(img,cv2.COLOR_BGRA2RGBA)
        cards[value].append((img,refCornerHL,refCornerLR))
        print(f"Nb images for {card_name} : {len(cards[value])}")

#BACK OF CARD
img=cv2.imread('pics/back.JPG',cv2.IMREAD_UNCHANGED)
_, img = extract_card(img, None, 120, False)
img=cv2.cvtColor(img,cv2.COLOR_BGRA2RGBA)
cards['back'] = [(img,refCard,refCard)]

#CHIPS
#cards['chip'] = []
for i in range(4):
    img = cv2.imread('pics/chip'+str(i+1)+'.JPG',cv2.IMREAD_UNCHANGED)
    _,img = extract_chip(img, None, 120, False)
    img=cv2.cvtColor(img,cv2.COLOR_BGRA2RGBA)
    #cards['chip'] += [(img,refChip,refChip)]
    display_img(img)
    plt.show()


# img = cv2.imread('pics/chip1.JPG',cv2.IMREAD_UNCHANGED)
# _,img = extract_chip(img, None, 120, True)
# img=cv2.cvtColor(img,cv2.COLOR_BGRA2RGBA)
...
def create_voc_xml(xml_file, img_file,listbba,display=False):
    with open(xml_file,"w") as f:
        ...
        for bba in listbba:
            # f.write(f"{card_values.find(bba.classname)} {bba.x1} {bba.y1} {bba.x2 - bba.x1} {}")
